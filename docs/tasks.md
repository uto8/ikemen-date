...
#### O-3: 男性オンボーディングフォーム
- [x] `components/profile/MaleOnboardingForm.tsx` を実装する
- [x] 女性フォームの項目に加えて occupation・height・bio・イケメンタイプ（12 種チェックボックス）を追加する
- [x] イケメンタイプを 0 件で送信したとき「1 つ以上選択してください」を表示する
...
- [x] `components/user-card/UserCard.tsx` を実装する
- [x] プロフィール画像（または女性のデフォルトアバター）・ニックネーム・年齢・居住地を表示する
- [x] 男性カードにはイケメンタイプバッジを 1 件表示する
- [x] カード全体を `<Link href="/users/[id]">` でラップする
...
#### W-2: 退会画面 UI
- [x] `app/(app)/settings/page.tsx` を作成する
- [x] 「退会する」ボタンと確認ダイアログ（`window.confirm` または モーダル）を実装する
- [x] 退会後チャットで「相手が退会しました」バナーが Realtime で受信できるようチャンネルイベントを設定する
...
- [x] `components/navigation/BottomNav.tsx` を実装する（タブ: ユーザー一覧 / もらったいいね / マッチング / 設定）
- [x] `app/(app)/layout.tsx` に組み込む
- [x] 現在のパスに対応するタブをアクティブ表示する
- [x] いいねタブに B-1 の未読いいね数バッジを表示する（いいね一覧表示中は非表示）
- [x] マッチングタブに B-2 の未読メッセージ数バッジを表示する（マッチング一覧表示中は非表示。クリアはチャット画面を開いたときのみ）
...
- [x] `npm install -D @playwright/test` を実行する
- [x] 認証フロー（登録 → メール確認 → オンボーディング → 一覧）のテストを作成する
- [x] いいね → マッチング → チャット開通のテストを作成する
- [x] 未マッチのチャット URL への直接アクセスでリダイレクトされるテストを作成する
